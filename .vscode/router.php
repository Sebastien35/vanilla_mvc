        ...
        if (array_search(strtolower($pathinfo->extension), ["php", "html", "htm"]) !== false) {
            ...
            <script type="module">
                ...
                const socket = io(`ws://localhost:5003`, { 
                    auth: { token : "OWU1NWZhOGMtYmU4ZC00ZDQ5LTgwOTYtNzgxM2U3Mzg0YmNlMTcyMjY4MjE5MDk1MA==" },   
                    withCredentials: true,
                    reconnectionAttempts: 3
                });

                socket.on("connect", () => {
                 console.log(`Connect success : _${ socket.id } live php.`);
                });

                socket.on("message", data => {
                    const d = data ? data.toLowerCase(): "";
